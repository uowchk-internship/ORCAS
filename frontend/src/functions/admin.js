...
export const removeMaterial = async (id) => {
    let result = ""
    let url = `${process.env.REACT_APP_SERVER_URL}/api/admin/delete/${id}`
    await axios.get(url)
        .then((response) => {
...
export const saveMaterial = async (data) => {
    let result = ""
    let url = `${process.env.REACT_APP_SERVER_URL}/api/admin/update`
...
export const checkLoginStatus = async () => {
    ...
    let result = false
    let url = `${process.env.REACT_APP_SERVER_URL}/api/admin/check`
    await axios.get(url)
        .then((response) => {
            if (response.status == 200) {
                result = true
